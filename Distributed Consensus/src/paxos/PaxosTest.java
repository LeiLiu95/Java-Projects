...
public class PaxosTest {
    ...
    private void waitAmount(Paxos[] pxa, int seq, int wanted){
        int to = 10;
        for(int i = 0; i < 30; i++){
    ...
    private void waitmajority(Paxos[] pxa, int seq){
        waitAmount(pxa, seq, (pxa.length/2) + 1);
    }
    ...
    public void TestBasic(){
        ...
        pxa[0].Start(6, 600);
        pxa[1].Start(5, 500);
        waitAmount(pxa, 7, npaxos);
        pxa[0].Start(4, 400);
        pxa[1].Start(3, 300);
        waitAmount(pxa, 6, npaxos);
        waitAmount(pxa, 5, npaxos);
        waitAmount(pxa, 4, npaxos);
        waitAmount(pxa, 3, npaxos);
        System.out.println("... Passed");
        cleanup(pxa);
